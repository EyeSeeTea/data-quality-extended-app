/**
 * Legacy implementation of QualityAnalysisRepository for DHIS2 ≤ 2.41.
 *
 * Differs from QualityAnalysisD2Repository only in the tracker API response
 * shape and query-parameter names:
 *   - Response: `.instances[]` (not `.trackedEntities[]`)
...
import {
    ...
    TrackedEntitiesGetResponseLegacy,
...
import { DataStore } from "@eyeseetea/d2-api-legacy/api";
import type { AsyncPostResponse } from "@eyeseetea/d2-api-legacy/api/common";
import type { TrackedOrderBase } from "@eyeseetea/d2-api-legacy/api/trackerTrackedEntities";
...
export class QualityAnalysisD2LegacyRepository implements QualityAnalysisRepository {
    ...
    get(options: QualityAnalysisOptions): FutureData<QualityAnalysisPaginated> {
        return apiToFuture(
            this.api.tracker.trackedEntities.get({
                ...
                // @ts-ignore - trackedEntities is the preferred param name in newer minor versions
    ...
    getById(id: string, metadata: MetadataItem): FutureData<QualityAnalysis> {
        return this.get({
            ...
            pagination: { page: 1, pageSize: 1e6 },
    ...
    save(qualityAnalysis: QualityAnalysis[], metadata: MetadataItem): FutureData<void> {
        ...
        return Future.sequential([$requests]).flatMap(() => Future.success(undefined));
    }
    ...
    private getAllD2TrackerTrackedEntities(
        ...
        const fetchPage = (
            ...
            return apiToFuture(
                ...
                const apiTeis = response.instances.map(tei => ({
    ...
    private getSectionInformation(teiIds: Id[], programStages: ProgramStage[]) {
        ...
        return Future.parallel($requests, { concurrency: 5 }).map(response =>
            _(response)
                .map(record => (record.extraInfo ? record : undefined))
                .compact()
                .value()
        );
    }
    ...
    private buildSectionsRequests(qaIds: string[], qualityAnalysis: QualityAnalysis[]) {
        ...
        const $requests = _(qaIds)
            .map(qualityId => {
                ...
                if (!qAnalysis || qAnalysis.sections.length === 0) return undefined;
                const sections = qAnalysis.sections.map(section => ({
                    id: section.id,
                    status: section.status,
                }));
                return apiToFuture(
                    dataStore.save(qualityId, { sections }).map(response => {
    ...
    private buildTeisRequests(
        ...
        return apiToFuture(
            ...
            const qualityAnalysisToPost = qaIds.map(qaId => {
                ...
                if (!qAnalysis) throw Error(`Cannot find qualityAnalysis: ${qaId}`);
...
type D2QualityAnalysisEntity = TrackedEntitiesGetResponseLegacy<
